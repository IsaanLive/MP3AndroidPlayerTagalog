...
public class DownloadingActivity extends ListActivity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if(totaldownloading >= 1){
        	text.setText("Kabuuang Download: "+totaldownloading);
        }else{
        	text.setText("Kabuuang Download: 0");
        }
        ...
        this.clearall.setOnClickListener(new OnClickListener() {
        	public void onClick(View v) {
        		...
        		toastmake("I-reset ang Nakumpleto & Nabigong Download");
        	}
        });
		...
		public View getView(int position, View convertView, ViewGroup parent){
	    	...
	    	if(items.get(position).third == 100){
	    		...
	    		if(a == 2){
		    		...
		    		if(a == 1){
		        		progress.setText("Nakabinbing pagre-Download");
		    		}else if(a == 0){
		    			progress.setText("Tapos Nagda-download");
		    		}else if(a == -1){
		        		progress.setText("Nabigong Nagda-download");
		    		}
	    		}
	...
	protected void onListItemClick(ListView l, View v, int position, final long id) {
		...
		if(items.get(position).third != 100){
			...
			if(downloaded == -1){
				...
				builder.setMessage("I-download Pagpipilian")
				       .setCancelable(false)
				       .setNegativeButton("Alisin", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				                refreshlist();
				                toastmake("MP3 Download Tinanggal");
				           }
				       })
				       .setNeutralButton("Magpilit na minsan pa", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   mDbHelper.updateDownloaded(ida, "1"); 
				        	   refreshlist();
				        	   toastmake("Itakda Upang Subukang muli");
				           }
				       })
				...
				final CharSequence[] items = {"Buksan MP3", "Itakda Bilang Ringtone", "Redownload", "Alisin", getString(R.string.btn_cancel_name)};
	
				AlertDialog.Builder builder = new AlertDialog.Builder(this);
				builder.setTitle("Pumili ng isang Pagpipilian");
				builder.setItems(items, new DialogInterface.OnClickListener() {
				    public void onClick(DialogInterface dialog, int item) {
				    	if(items[item] == "Buksan MP3"){
				    	    try {
				    	        //launch intent
				    	    	...
				    	    	toastmake("Error sa pag-Nilo-load ang MP3");
				    	        e.printStackTrace();
				    	    }
				    	}else if(items[item] == "Itakda Bilang Ringtone"){
				    	...
				    	}else if(items[item] == "Alisin"){
							AlertDialog.Builder builder = new AlertDialog.Builder(DownloadingActivity.this);
							builder.setMessage("Sigurado ka bang gusto mong tanggalin ang: " + title + " ?")
							       .setCancelable(false)
				...
				builder.setMessage("I-download Pagpipilian")
				       .setCancelable(false)
				       .setNegativeButton("MP3 Download Tinanggal", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   if(sid == 1){
				                ...
				                toastmake("Itigil ang I-download");
				           }
				       })
				       .setPositiveButton(getString(R.string.btn_cancel_name), new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
    ...
    public void refreshlist(){
    	...
    	if(a.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102109, "Kasalukuyang Nagda-download", 100));
    		a.moveToFirst();
    	...
    	if(a1.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102110, "Nakabinbing pagre-Download", 100));
    	...
    	if(a11.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102111, "Tapos Downloads", 100));
    	...
    	if(a111.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102112, "Nabigong Download", 100));
